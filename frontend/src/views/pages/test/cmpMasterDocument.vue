...
export default {
  ...
  data() {
    return {
      ...
      template: [],
      mappingInputs: [], // Data input dinamis
    };
  },
  async mounted() {
  ...
  watch: {
    selectedtemplate(newValue) {
      this.onChangeTemplate(newValue);
    },
  },
  ...
  methods: {
    async getTemplate() {
      try {
        const response = await axios.get(
          this.$root.apiHost + "api/doclistalldata"
        );

        // Ubah data menjadi format array yang diterima v-select
        console.log(response.data);

        // this.template = response.data.data.map((item) => ({
        //   label: item.template_name,
        //   value: item.id,
        //   inputs: item.dynamicInputs || [], // Data input dinamis jika ada
        // }));
    ...
    //     this.todo.template_name = this.selectedtemplate; // Set template_name sesuai template yang dipilih
    //   } else {
    //     this.todo.template_name = "";
    //   }
    // },

    onChangeTemplate(selected) {
      if (selected) {
        this.todo.template_name = selected; // Set template_name
        this.mappingInputs = selected.inputs; // Set input dinamis
      } else {
        this.todo.template_name = "";
        this.mappingInputs = []; // Reset jika tidak ada template yang dipilih
      }
    },
    padTo2Digits(num) {
