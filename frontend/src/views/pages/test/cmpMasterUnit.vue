        ...
        <div class="modal-body">
            ...
            <div class="col-md-12">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Nomor Unit</label>
                  <CmpInputText
                    id="inputA"
                    type="number"
                    placeholder="Nomor Unit"
                    v-model="todo.unit_number"
                    :class="
                      errorField.unit_number
                        ? 'form-control input-lg input-error'
                        : 'form-control input-lg'
                    "
                    @input="
                      (val) =>
                        (todo.unit_number = todo.unit_number.toUpperCase())
                    "
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Kode Unit</label>
                  <CmpInputText
                    type="text"
                    placeholder="Kode Unit"
                    v-model="todo.unit_code"
                    :class="
                      errorField.unit_code
                        ? 'form-control input-lg input-error'
                        : 'form-control input-lg'
                    "
                    @input="
                      (val) => (todo.unit_code = todo.unit_code.toUpperCase())
                    "
                  />
                </div>
              </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="example-nf-email">Nama Unit</label>
                  <CmpInputText
                    id="inputA"
                    type="text"
                    placeholder="Nama Unit"
                    v-model="todo.unit_name"
                    :class="
                      errorField.unit_name
                        ? 'form-control input-lg input-error'
                        : 'form-control input-lg'
                    "
                    @input="
                      (val) => (todo.unit_name = todo.unit_name.toUpperCase())
                    "
                  />
                </div>
              </div>
  ...
  methods: {
      ...
      while (count > 0) {
        ...
        Object.keys(resData.results).forEach(function (key) {
          const countries_x = {
            ...
            unit_number: "'" + resData.results[key].unit_number,
            unit_code: resData.results[key].unit_code,
            unit_name: resData.results[key].unit_name,
          };
    ...
    saveTodo() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          // };
          const config = "";
          var url = mythis.$root.apiHost + "api/hr_unit";
          axios
            .post(
              url,
              {
                unit_number: mythis.todo.unit_number,
                unit_code: mythis.todo.unit_code,
                unit_name: mythis.todo.unit_name,
    ...
    getTable() {
      ...
      this.grid.updateConfig({
        ...
        server: {
          url: this.$root.apiHost + "api/hr_unit",
          then: (data) =>
            data.results.map((card) => [
              card.id,
              data.nomorBaris++ + 1,
              html(`<span class="pull-left">${card.unit_number}</span>`),
              html(`<span class="pull-left">${card.unit_code}</span>`),
              html(`<span class="pull-left">${card.unit_name}</span>`),
            ]),
    ...
    deleteTodo(id) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          // const config = "";
          axios
            .delete(mythis.$root.apiHost + `api/hr_unit/${id}`, config)
            .then((res) => {
    ...
    editTodo() {
      ...
      axios
        .put(
          mythis.$root.apiHost + "api/hr_unit/" + mythis.todo.id,
          {
            unit_number: mythis.todo.unit_number,
            unit_code: mythis.todo.unit_code,
            unit_name: mythis.todo.unit_name,
    ...
    async getData(id) {
      ...
      await axios
        .get(mythis.$root.apiHost + `api/hr_unit/${id}`, config)
        .then(async (res) => {
          //console.log(res.data.data);
          //mythis.acuanEdit = id;
          //mythis.todo = res.data.data;
          mythis.todo.id = id;
          mythis.todo.unit_number = res.data.data.unit_number;
          mythis.todo.unit_code = res.data.data.unit_code;
          mythis.todo.unit_name = res.data.data.unit_name;
